...
function preload_experiment(){
    // cfg here only holds the url parameters
    if(cfg.hasOwnProperty('group')) {
        if (debug_preload) console.log("Using group token");
        preload_state='pre_group'
    } else {
        if (debug_preload) console.log("Using session token");
        preload_state='pre_session';
    }
    preload_start=window.performance.now();
    ctx.font = "24px Arial";
    ...
    requestId=window.requestAnimationFrame(preload_draw);
    if (debug_preload) console.log("Back to preload_experiment");
    // return sessiontoken, although it should already be set globally
    return true;
}
